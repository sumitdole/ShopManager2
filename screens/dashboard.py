...
import utils
from widgets import StatCard, PRIMARY, SUCCESS, WARNING, DANGER
...
class DashboardScreen(MDBoxLayout):
    def __init__(self, app, **kwargs):
        ...
        actions.add_widget(MDRaisedButton(
            text="View Products", size_hint_x=1, md_bg_color=self._muted_color(),
            on_release=lambda *_: self._goto("products"),
        ))
        actions.add_widget(MDRaisedButton(
            text="View Inventory / Low Stock", size_hint_x=1, md_bg_color=self._muted_color(),
    ...
    def _muted_color(self):
        from widgets import hex_rgba
        return hex_rgba("64748B")
